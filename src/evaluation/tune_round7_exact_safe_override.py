...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "outputs" / "models" / "round7_exact_safe_override"
DEFAULT_PREDICTION_DIR = PROJECT_ROOT / "outputs" / "predictions"
DEFAULT_REPORT_MD = PROJECT_ROOT / "outputs" / "evaluation" / "round7_exact_safe_override_tuning_report.md"
...
def baseline_metrics_by_split(rows: Sequence[Dict]) -> Dict[str, Dict]:
    return {
        split_name: metrics_for(
            split_rows_group,
            [int(row["step7_pred"]) for row in split_rows_group],
            [prob(row, "step7_prob") for row in split_rows_group],
        )
        for split_name, split_rows_group in split_rows(rows).items()
    }
...
def bucket_allowed(row: Dict, rules: Dict) -> bool:
    ...
    if bucket in policy["general"]:
        return (
            prob(row, "p_round7_safe_override") >= float(rules["general_p_round7_safe_min"])
...
def apply_rules(rows: Sequence[Dict], rules: Dict) -> Tuple[List[int], List[float], List[Dict]]:
    ...
    for row in rows:
        ...
        should_override = (
            ...
            and bucket_allowed(row, rules)
            and prob(row, "p_unsafe_override", 1.0) <= float(rules["p_unsafe_max"])
            and prob(row, "guard_p_human_style", 1.0) <= float(rules["human_style_max"])
            and prob(row, "p_round7_safe_override") >= float(rules["p_round7_safe_min"])
        ...
        decisions.append(
            {
                "round7_override": bool(should_override),
                "round7_policy": rules["bucket_policy"],
                "round7_bucket": str(row.get("round4_bucket") or row.get("bucket") or "unknown"),
            }
        )
...
def candidate_rules(args) -> List[Dict]:
    rules = [
        {
            "p_round7_safe_min": 1.1,
            "p_unsafe_max": -1.0,
            "human_style_max": -1.0,
            "round4_threshold": 1.1,
            "min_delta": 1.1,
            "bucket_policy": "disabled",
            "general_p_round7_safe_min": 1.1,
    ...
    for p_safe, p_unsafe, human_style, round4_threshold, min_delta, policy in itertools.product(
        float_grid(args.p_round7_safe_min),
        float_grid(args.p_unsafe_max),
        float_grid(args.human_style_max),
        ...
        rules.append(
            {
                "p_round7_safe_min": p_safe,
                "p_unsafe_max": p_unsafe,
                "human_style_max": human_style,
                "round4_threshold": round4_threshold,
                "min_delta": min_delta,
                "bucket_policy": policy,
                "general_p_round7_safe_min": min(0.99, p_safe + args.general_strict_p_safe_margin),
...
def evaluate_candidate(rows: Sequence[Dict], rules: Dict, baseline: Dict[str, Dict], args) -> Dict:
    ...
    for split_name, group in split_rows(rows).items():
        indices = [index for index, row in enumerate(rows) if (row.get("split") or row.get("split_name")) == split_name]
        split_preds = [preds[index] for index in indices]
        split_probs = [probs[index] for index in indices]
    ...
    if args.internal_split in by_split:
        ...
        constraints.append(by_split[args.internal_split]["false_positives"] <= baseline[args.internal_split]["false_positives"] + args.internal_fp_tolerance)
    ...
    for split_name, delta in deltas.items():
        if split_name == args.hardpos_split:
            ...
            score -= 30.0 * delta["induced_fp"] + 0.05 * delta["overrides"]
        elif split_name == args.internal_split:
            score += 2.0 * delta["fixed_step7_fn"] - 25.0 * delta["induced_fp"] - 0.02 * delta["overrides"]
    return {
        "rules": rules,
        "score": float(score),
...
def write_report(report: Dict, path: Path) -> None:
    ...
    lines = [
        "# Round7 Exact Safe Override Tuning Report",
        "",
        "Teacher-test labels are not used here. Step7 remains the default prediction.",
        "This two-stage search blocks obvious unsafe rows with Round5/human-style guards before allowing high-confidence Round7 exact-selector overrides.",
        "",
        "## Selected Rule",
        "",
        "```json",
    ...
    lines.extend(["", "## Override Delta", "", "| Split | Overrides | Fixed Step7 FN | Induced FP | Broke Step7 Correct |", "| --- | ---: | ---: | ---: | ---: |"])
    ...
    lines.extend(["", "## Search Summary", "", f"- candidate rules: {report['num_candidates']}", f"- feasible rules: {report['num_feasible']}", "", "## Decision", "", "```text", report["decision"], "```", ""])
    path.parent.mkdir(parents=True, exist_ok=True)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Tune Round7 exact-selector local override rules.")
    ...
    parser.add_argument("--min_internal_f1", type=float, default=0.9570)
    parser.add_argument("--internal_fp_tolerance", type=int, default=1)
    ...
    parser.add_argument("--min_hardpos_fixed_fn", type=int, default=57)
    parser.add_argument("--p_round7_safe_min", default="0.45,0.50,0.55,0.60,0.65,0.70,0.75,0.80")
    parser.add_argument("--p_unsafe_max", default="0.20,0.25,0.30,0.35,0.40,0.45")
    parser.add_argument("--human_style_max", default="0.65,0.75,0.80,1.00")
    parser.add_argument("--round4_threshold", default="0.50,0.55,0.60,0.65")
    parser.add_argument("--min_delta", default="0.00,0.05,0.10")
    parser.add_argument("--bucket_policy", default="old_short,short_only,general_strict,old_short_plus_general_strict")
...
def main():
    ...
    for values in args.tune_set:
        ...
        for row in load_jsonl(path):
            ...
            rows.append(item)
    if not rows:
        raise ValueError("No Round7 tuning rows found.")
    ...
    tuned_files = {}
    for split_name, group in split_rows(rows).items():
        indices = [index for index, row in enumerate(rows) if row.get("split") == split_name]
        out_path = prediction_dir / f"round7_exact_safe_override_{split_name}_predictions.jsonl"
        save_jsonl(
            prediction_rows(
                group,
                [preds[index] for index in indices],
    ...
    decision = (
        ...
        else "PROMOTE_TO_ROUND7_GATE_REPORT = no; keep Step7 and improve exact-selector data or rules."
    )
    report = {
        "selected": selected,
    ...
    print("=" * 70)
    print("Round7 exact safe override tuning complete")
    print("=" * 70)
